...
if(isset($_POST['Submit']))
 ...
 // echo "<pre>"; print_r($_POST); echo "</pre>";
	...
	$conn=mysqli_connect('localhost','root','') or die("enable to connect".mysqli_error($conn));
    mysqli_select_db($conn,'myroject_db') or die("can't select database".mysqli_error($conn));
    echo   "insert into tb_video(class,caption,url,file)values('$class','$caption','$url','$homewrokFile')";

	$q="insert into tb_video(class,caption,url,date,session,status)values('$class','$caption','$url','$date','$session','1')";
				$rs=mysqli_query($conn,$q);
				if($rs>0)
				{
					 $messag= "Uploades Successfully";
	...
	$q="insert into tb_youtube_video(title,link,status)values('$title','$link','1')";
				$rs=mysqli_query($conn,$q);
				if($rs>0)
				{
					 $messag= "Uploades Successfully";
	...
	<form id="add-videos" class="form-horizontal" action="#" method="POST" enctype="multipart/form-data">
		<div class="panel panel-info">
				...
				<div class="col-sm-3">
						<label class="control-label col-sm-2" style="text-align:left">Class</label>
							<select name="class" class="form-control" style="z-index:1000;">
								<option value="">--</option><option value="I">I</option><option value="II">II</option><option value="III">III</option><option value="IV">IV</option><option value="V">V</option><option value="VI">VI</option><option value="VII">VII</option><option value="VIII">VIII</option><option value="IX">IX</option><option value="X">X</option><option value="IX">IX</option><option value="X">X</option><option value="XI">XI</option><option value="XII">XII</option>              </select>
				</div>
					<div class="col-sm-3">
			...
			<div class="panel-footer">
				<center>
					<button type="submit" name="Submit" value="AddVideo" id="videoss" class="btn btn-primary btn-md">Submit  </button>
	...
	<form id="add-videos" class="form-horizontal" action="#" method="POST" enctype="multipart/form-data">
		<div class="panel panel-info">
			<div class="panel-heading">
				<div class="row">
					<div class="col-md-6 text-left">
						<strong><span class="glyphicon glyphicon-facetime-video"></span> Add Video Gallery</strong>
					</div>   
				</div>
			</div>
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
					<label class="control-label col-sm-5" for="caption" style="text-align:left">Enter Title</label>
						<input type="text" class="form-control" name="title" id="caption" placeholder="Caption" required="">
					</div>
					<div class="col-sm-3">
					<label class="control-label col-sm-2" for="url" style="text-align:left">URL</label>
						<input type="text" class="form-control" name="link" id="url" placeholder="URL" required="">
					</div>
			...
			<div class="panel-footer">
				<center>
					<button type="submit" name="Submit1" value="AddVideo" id="videoss" class="btn btn-primary btn-md">Submit  </button>
					<button type="reset" class="btn btn-danger btn-md">Reset</button>
				</center>
			</div>
		</form>  <!--.panel-footer-->
